 ...
 * It may have been more efficient if it worked one line at time.
 * However, that would have required extensive modifications to more.c.
 * Simply using Getc() and Ungetc() required almost no modifications to
 * more.c, since it already used them for everything.
 *
 * This still reads as fast as a terminal can display it, so efficiency
 * is not really an issue.
 *
 * When writing the mfile data type, I considered using a temporary file
 * to store the data that is read.
 * But I chose to use memory instead, because I consider it unlikely
 * that the user will pipe too much data to "more".
 *
 * Of course, the "mfile" routines still can be rewritten to use a
 * temporary file, without changing the data type specifications at all.
 */
